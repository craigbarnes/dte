...
LDFLAGS ?=
AWK = awk
VERSION = $(shell mk/version.sh 1.10)
...
BUILTIN_SYNTAX_FILES ?= \
    awk c config css d diff docker dte gcode gitcommit gitrebase go gomod html \
...
util_objects := $(call prefix-obj, build/util/, \
    array ascii base64 debug exec hashmap hashset intern intmap log \
    numtostr path ptr-array readfile string strtonum unicode utf8 \
    xmalloc xmemmem xreadwrite xsnprintf )
...
test_objects := $(call prefix-obj, build/test/, \
    ...
    encoding filetype history main options spawn syntax terminal test util )

bench_objects := $(call prefix-obj, build/test/, benchmark)

feature_tests := $(addprefix build/feature/, $(addsuffix .h, \
    dup3 pipe2 fsync memmem TIOCGWINSZ TIOCNOTTY tcgetwinsize posix_madvise ))
